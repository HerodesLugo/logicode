    ...
    <section className="w-full bg-brand-light-bg py-20 lg:py-[84px]">
      <div className="section-container flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-4 max-w-[760px]">
          ...
          <h2 className="text-gray-950 text-3xl lg:text-[32px] font-bold">
            From threat intel to incident resolution.
          </h2>
          <p className="text-gray-700 text-sm leading-relaxed max-w-[740px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-[20px] p-8 flex flex-col gap-[60px]">
            {/* Card header */}
            <div className="flex flex-col gap-4">
              <h3 className="font-display font-semibold text-xl text-brand">
            ...
            <div className="flex flex-col gap-8">
              {steps.map((step) => (
                <div key={step.number} className="grid grid-cols-[64px_1fr] gap-[18px]">
                  {/* Number badge */}
                  <div className="flex-shrink-0 w-16 h-16 rounded-[20px] bg-[rgba(40,240,106,0.1)] flex items-center justify-center">
                    <span className="text-brand text-2xl font-normal">
                      {step.number}
                    </span>
                  </div>
                  {/* Text */}
          ...
          {/* Right card: metrics */}
          <div className="bg-white rounded-[20px] p-8 flex flex-col justify-between gap-8">
            {/* Card header */}
            <div className="flex flex-col gap-4">
              <h3 className="font-display font-semibold text-xl text-brand">
                Operational impact
            ...
            <div className="flex flex-col gap-8">
              {metrics.map((metric) => (
                <div key={metric.value} className="flex flex-col gap-1">
                  <span className="text-brand text-5xl font-normal leading-none">
                    {metric.value}
                  </span>
                  <span className="font-display font-semibold text-sm text-[#16211b] mt-2">
                    {metric.label}
                  </span>
                  <p className="text-gray-700 text-xs leading-relaxed max-w-[520px]">
                    {metric.description}
                  </p>
                </div>
              ))}
